function [Lambda, Phi, Bhat, W, Uhat, Utilde1, Utilde2, Stilde, Vtilde,...
    zeroInd, Atilde] = ...
    func_DMDc(X1, X2, Upsilon, r, rtilde, B)
...
%         Utilde2 = Utilde(end-rtilde+r+1:end, 1:rtilde);
        % MY MODIFICATIONS
        %   I'm pretty sure the number of rows in the tilde matrices should
        %   simply split them back up as Omega was formed and have nothing
        %   to do with the truncations
        if rtilde==0
            rtilde = optimal_truncation(Omega, 'rank');
        end
        Utilde1 = Utilde(1:size(X1,1), 1:rtilde);
        Utilde2 = Utilde(size(X1,1)+1:end, 1:rtilde);
        if r==0
